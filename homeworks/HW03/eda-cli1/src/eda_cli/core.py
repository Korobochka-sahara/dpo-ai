...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, df: pd.DataFrame) -> Dict[str, Any]:
    ...
    zero_heavy_cols = [
        ...
        if (df[col] == 0).mean() > ZERO_SHARE_THRESHOLD
    ]
    flags["zero_value_columns"] = len(zero_heavy_cols)
    ...
    # новые эвристики
    score -= 0.05 * flags.get("constant_columns", 0)
    score -= 0.05 * flags.get("duplicate_ids", 0)
    score -= 0.02 * flags.get("zero_value_columns", 0)
